...
// The app loads the same table as a script rather than fetching the JSON.
// Precomputed rather than derived live from the loaded 5etools data for the
// same reason token-index.js is: the turn view needs it the instant it mounts,
// the class files are loaded lazily, and re-running a hundred regexes over
// every class feature on every render to rediscover a constant would be work
// done again and again for an answer that only changes when the data does.
const js = path.join(__dirname, '..', 'js', 'generated', 'reactions.js');
fs.writeFileSync(js,
  '// GENERATED by tools/extract-reactions.js — do not hand-edit.\n'
  + '// Which features are reactions, read out of data/class/*.json, data/feats.json\n'
  + '// and data/spells/*.json rather than out of anyone\'s memory of the PHB.\n'
  + '// ' + classes.length + ' come from base classes, ' + subclasses.length + ' from subclasses, '
  + feats.length + ' from feats, ' + spells.length + ' are reaction spells.\n'
  + '// Deriving from class and level alone would find ' + classes.length + ' of '
  + (classes.length + subclasses.length + feats.length) + ' — which is why subclass is not optional.\n'
  + 'window.SKT_REACTIONS = ' + JSON.stringify(out) + ';\n');
const jskb = Math.round(fs.statSync(js).size / 1024);
...
console.log('-> ' + path.relative(process.cwd(), js)   + '  (' + jskb + ' KB)');
